...
2. **Standards 轴**（平台规范合规）：
   - `../../reference/rn/rn-guidelines.md` 合规（label 可见性、dependencies 禁用、动态表单安全等后续扫描步骤的结论可作为证据引用）
...
```bash
node <skill目录>/scripts/dtc-artifacts.mjs check ".ai-wiki/【需求名】" --platform rn --mode "${inputs.mode:-standard}"
```
