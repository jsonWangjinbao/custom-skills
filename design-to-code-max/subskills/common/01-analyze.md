...
3 个问题回答完毕后，推进 `currentPhase` 到 `collect-materials`，fallthrough 到 `subskills/common/02-collect-materials.md`。
...
新需求流程完成后，输出需求名称、平台和需求类型摘要，使用 `AskUserQuestion` 确认：

```
问题：需求初始化完成：「门店证件管理」[RN] + [重构]。是否确认进入材料收集？
选项：
- 确认，进入材料收集
- 重新选择（我会输入修改意见）
```
