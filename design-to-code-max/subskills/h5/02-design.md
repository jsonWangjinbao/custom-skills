...
- 新增页面必须在集中式路由配置 `src/config/route.ts` 中注册
...
对「黑盒封装组件」，参考 `../../reference/common/gotchas/blackbox-wrapper-component.md`（黑盒组件识别方法通用）。
...
引用 `../../reference/common/gotchas/` 下已知问题中与 H5 Web 端相关的部分（跨平台差异、WebView 注意事项等）。
...
- [ ] 路由已注册到集中式路由配置 `src/config/route.ts`
...
1. **必须停下来向用户确认**：输出文件结构、路由、API 调用、组件决策 summary，使用 `AskUserQuestion` 询问：
   ```
   问题：H5 技术设计完成，共设计 N 个文件、N 条路由、N 个 API。是否确认开始生成代码？
   选项：
   - 确认，开始生成代码
   - 需要调整（我会输入修改意见）
   ```
