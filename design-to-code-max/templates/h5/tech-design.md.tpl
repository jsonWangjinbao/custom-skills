...
- **路由注册**: {{路由路径}} → 已在 `src/config/route.ts` 的 `FsmsRouteKeys` 中注册 key
...
- **间距**: `var(--xlb-space-*)`
- **图标**: 逐条写出图标决策
  - {{图标 1 语义}} → `<XlbIcon type="{{IconKey}}" size="{{size}}px" color="var(--xlb-color-*)" />`
  - {{图标 2 语义}} → `<XlbIcon type="{{IconKey}}" size="{{size}}px" color="var(--xlb-color-*)" />`
- **图片**: 逐条写出图片资源决策
...
- **PostCSS rem 转换**: 设计稿 750px → px 值 → PostCSS pxtorem (rootValue: 75) 自动转换，SCSS 中始终写设计稿原始 px 值
