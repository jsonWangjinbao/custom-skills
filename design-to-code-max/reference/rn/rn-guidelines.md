...
> 本规范与 `reference/rn/xlb-style-system.md` 互补：本文件管"HTML → RN 的转换规则"，xlb-style-system.md 管"RN 样式在 XLB 项目中怎么写"。生成代码前必须同时读两份文件。
...
> 完整映射 → `reference/rn/token-map.json`。设计稿中的 hex 必须先在 token-map 中查找对应 key；找不到时禁止写死 hex，应在 `ui-audit.md` 中标记「无 token 映射」并在技术设计阶段提出风险。
...
> ⚠️ `name` 必须与 `iconfontGlyphMap` 的 key **完全一致**。常见错误是把 `Search` 写成 `icon_search`，会渲染为 `null`。完整映射见 `reference/rn/icon-map.md`。
...
1. 在 `reference/rn/icon-map.md` 中找最接近的语义替代
...
数组 `name` 生成的 `id` 传到原生 Android TextInput 映射为 `nativeID`，Android 期望 String 收到 Array 崩溃。受影响组件：`XlbInput`、`XlbUploadFile`。不受影响：`CommonFormItem type="date"/"selector"`。详见 `reference/rn/gotchas/component-library/safeinput-filter-id.md`。
...
> ⚠️ 这是最容易出错的坑。只改 remote app 不够，host app 的 Navigator 中 Screen 是通过 CDN router.json 动态注册的。完整步骤和验证清单见 `reference/rn/gotchas/rn-quirks/module-federation-route-registration.md`
