...
> 把设计稿（HTML/截图）转成 RN/H5/PC 生产代码的 skill。
...
**这个 skill 就是把这些问题固化成一套流程：** 不管谁用、不管什么时候用，AI 都走同一条分阶段流水线，产出标准文档，做标准检查。
...
1. **回答问题** — AI 会一步步问你需求名称、目标平台（RN/H5/PC）、类型、设计稿在哪
...
```
入口（选平台+类型）→ 材料收集 → 功能点 → API规格 → UI审计 → 技术设计 → 生成代码 → 自测 → 交付
       └────────── 通用阶段，三端共用 ──────────┘   └────── 按平台走 RN / H5 / PC 各自流水线 ──────┘
                        前 6 个阶段都要你确认，生成代码后还有出口门禁和终检
```
...
```
design-to-code-max/
├── SKILL.md                  ← 总纲（8 条铁律 + 路由流程）
├── subskills/                ← 分阶段执行说明书（通用 + 分平台）
│   ├── common/               ← 通用阶段：入口、材料收集、功能点、API规格
│   ├── rn/                   ← RN 流水线：UI审计、技术设计、生成代码、自测
│   ├── h5/                   ← H5 流水线（同上 4 阶段）
│   └── pc/                   ← PC 流水线（同上 4 阶段）
├── reference/                ← 知识库（AI 写代码时的参考依据）
│   ├── common/               ← 通用规则（HTML 解析、歧义检测）
│   ├── rn/                   ← RN 规范 + token 映射 + gotchas 踩坑记录
│   ├── h5/                   ← H5 规范（CSS 变量体系）
│   └── pc/                   ← PC 规范 + 组件映射
...
└── 【需求名】/
    ├── features.md                     ← 功能点清单
    ├── api-spec.md                     ← API 规格（入参/出参/取值路径/mock）
    ├── ui-audit.md                     ← UI 审计报告
    ├── tech-design.md                  ← 技术设计
...
- 先收集材料、拆功能点 → 出功能点清单
- 再定 API 规格 → 出接口入参/出参/mock 模板
- 再审计 UI → 出样式规格
- 再技术设计 → 出组件架构
- 再分组生成 → 每组做完做模式对齐 + 偏差检查
- 最后全面自测 → 过门禁才交付
